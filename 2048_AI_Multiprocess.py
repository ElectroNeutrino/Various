...
# Define number of input nodes (number of game cells, must be 16)
node_in = 16
# Define size of internal nodes
node_1 = 32
# Define number of output nodes (number of directions, must be 4)
node_out = 4
...
game_board = np.zeros((4, 4), dtype=np.int64)
display_board = np.zeros((4, 4), dtype=np.int64)
...
def random_value(start = False):
    ...
    exclude_list = []
    while continue_game and len(exclude_list) < 16 and not (game_board[np.unravel_index(i,(4,4))] == 0):
    ...
    is_four = (np.random.randint(0,9) == 0)
...
def sigmoid(input_x):  
    return np.exp(-np.logaddexp(0, -input_x))
...
def calc_array(intermediate_value_array, output_value_array, bias):
    a_1 = np.zeros((pop_count, 16, 4), dtype=np.int64)
    a_2 = np.zeros((pop_count, 4), dtype=np.int64)
...
def calculate_move(a_1, a_2, pop):
    input_values = np.zeros(node_in, dtype=np.float64)
    ...
    ai_direction = input_type[np.argmax(output_values)]

    return ai_direction

# Play individual members of the population with AI
def run_single_pop(a_1, a_2, score_queue, steps_queue, j):
    global pop_count

    # initialize empty process internal arrays
    best_index = np.zeros((pair_count, 3), dtype=np.float64)
    average_score = np.zeros((pop_count, 1), dtype=np.float64)
    average_steps = np.zeros((pop_count, 1), dtype=np.float64)

    for pop in [k for k in range(pop_count) if (k%4==j)]:
        running_score = 0
        running_steps = 0
        input_values = np.zeros((node_in, 1), dtype=np.float64)
...
#################
...
elif input_source == "AI" and __name__ == '__main__':
    ...
    intermediate_value_array = np.random.rand(pop_count, node_in, node_1) #input to first level
    output_value_array = np.random.rand(pop_count, node_1, node_out) #second level to output

    bias = np.random.rand(pop_count, node_1) #bias from input to first level

    print ("Number of processess:", process_count)
    ...
    for gen in range(gen_max):
        ...
        # calculate AI arrays (a_1 * input + a_2 = output)
        a_1, a_2 = calc_array(intermediate_value_array, output_value_array, bias)

        # initalize return value queues for multiprocess comminucation
        average_score_queue = SimpleQueue()
        average_steps_queue = SimpleQueue()

        # spin off population runs into n separate processes
        running_processes = []
        ...
        while not average_score_queue.empty():
            average_score += average_score_queue.get()
        while not average_steps_queue.empty():
            average_steps += average_steps_queue.get()
        ...
        if ((gen+1)%10 == 0) or gen == (gen_max-1):
            ...
            print('\n')
            try:
                np.savetxt("time.csv", total_elapse, delimiter=",")
            except:
...
#                index_x = int(best_index[x][0])
                ...
                intermediate_value_array[calc_index] = (new_IIA[index_x] + np.random.rand(node_in, node_1)) * 0.5
                output_value_array[calc_index] = (new_IOA[index_x] + np.random.rand(node_1, node_out)) * 0.5

                bias[calc_index] = (new_bias[index_x] + np.random.rand(node_1)) * 0.5
                calc_index += 1

            while calc_index < pop_count: #fill rest with random
                intermediate_value_array[calc_index] = np.random.rand(node_in, node_1)
                output_value_array[calc_index] = np.random.rand(node_1, node_out) * 0.5

                bias[calc_index] = np.random.rand(node_1) * 0.5
                calc_index += 1
