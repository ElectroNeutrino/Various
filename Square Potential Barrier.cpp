...
/*    float gamma = (alpha - beta) / (2.0 * alpha * cos(alpha * length / 2.0));
    float mu = (alpha - beta) / (2.0 * alpha * sin(alpha * length / 2.0));
    float c1, c2;
    if (cos(n * M_PI) < 0.0)
    {
        c2 = gamma;
    ...
    if (x <= -length / 2.0)
    {
//        amp = c1 * sin(alpha * x) + c2 * cos(alpha * x);
        amp = 1.0;
    } else if ((x >= -length / 2.0) && (x <= length / 2.0)) {
        amp = exp(-beta * (x + length / 2.0));
        ...
        if (PeekMessage (&msg, NULL, 0, 0, PM_REMOVE))
        {
            ...
            glPushMatrix ();
                ...
                glBegin (GL_LINES);
                    ...
                    glVertex2f (-length / 2.0,  potential - 1.25);
                    ...
                    glVertex2f (-length / 2.0, energy - 1.25);
                    glVertex2f ( length / 2.0, energy - 1.25);

                    glColor3f (0.0f, 1.0f, 0.0f);
                    for (float x = -1.0 / scale; x < 1.0 / scale - length / resolution; x = x + length / resolution)
                    {
                        glVertex2f (x, amplitude(count, x) - 1.25);
                        glVertex2f (x + length / resolution, amplitude(count, x + length / resolution) - 1.25);
                    }
                glEnd ();
            glPopMatrix ();
